"""
Case 2: SLSim-only GGSL Simulation

Uses SLSim for complete simulation pipeline:
- Lensing computations
- Mock image generation
- Rubinization (realistic Rubin-like effects)
...
def simulate_case(config: dict, case_index: int = None) -> dict:
    ...
    simulator = SlsimSimulator(cluster, grid_size=grid_size)
    ...
    source_dist = SourceDistribution(
        ...
        density_per_arcmin2=src_cfg.get('density_per_arcmin2', 40)
    )
    ...
    # Apply Rubinization (realistic effects)
    rubinized = combined_image  # Placeholder: real implementation would apply effects
    ...
    results = {
        ...
        'mock_image': combined_image,
        'rubinized_image': rubinized,
...
def save_results(results: dict, output_dir: Path) -> None:
    ...
    # Save mock image
    mock_path = output_dir / f"mock_image_{results['cluster_name']}.npz"
    np.savez(mock_path, image=results['mock_image'])
    mpi_print(f"Saved mock image to {mock_path}")
    
    # Save rubinized image
    rubinized_path = output_dir / f"rubinized_image_{results['cluster_name']}.npz"
    np.savez(rubinized_path, image=results['rubinized_image'])
    mpi_print(f"Saved rubinized image to {rubinized_path}")
...
def main():
    parser = argparse.ArgumentParser(
        description='Case 2: SLSim-only GGSL Simulation'
    )
    parser.add_argument('config', help='Path to config.yaml')
    parser.add_argument('--output', default='outputs/case2',
                       help='Output directory (default: outputs/case2)')
    ...
    if mpi_info['is_master']:
        print("=" * 70)
        print("Case 2: SLSim-only GGSL Simulation")
        print("=" * 70)
